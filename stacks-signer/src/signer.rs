...
impl Signer {
    ...
    pub fn from_configs(config: &GlobalConfig, reward_cycle_config: RewardCycleConfig) -> Self {
        ...
        let signer_db = SignerDb::new(&config.db_path).expect("Failed to connect to signer Db");
        Self {
            coordinator,
            signing_round,
            state: State::Idle,
    ...
    fn process_signature(&mut self, signature: &Signature) {
        ...
        self.signer_db
            .remove_block(&signer_signature_hash)
        ...
        match e {
            ...
            SignError::InsufficientSigners(malicious_signers) => {
                ...
                let block = read_next::<NakamotoBlock, _>(&mut &message[..]).ok().unwrap_or({
                        ...
                        let Some(block_info) = self.signer_db.block_lookup(&signer_signature_hash).expect("Failed to connect to signer DB") else {
